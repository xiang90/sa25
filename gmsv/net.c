...
{
    BOOL            acceptmore;     /*  1蛍堡俔晩�accept 愼俔悗怜
                                        俳緒畔謄�close 塋埖 */
    unsigned int    fdid;           /*  fd 式彦垰粟 */
    unsigned int    closeallsocketnum;  /*   closeallsocket   式磨垰式
                                             佰*/
	int				shutdown;		/*  飯＆弥谷shutdown塋埖胴＆囁 
									 *	0:彦蛉 巷直強遜:橡賭喟傲橡賭喟傲胴＆囁
									 * 胴＆囁怨悧堡俔掃筑札  堡晒埖�
									 */
	int				dsptime;		/* shutdown 胴＆囁式轡＆柩  佰*/
	int				limittime;		/* 叶直返 */
}ServerState;
typedef struct tagCONNECT
{
  ...
  struct sockaddr_in sin; /* 摸糎枡式払囁卅稀 */
  ConnectType ctype;       /* 隷伏柩橡呀周式屠 */

  char cdkey[ CDKEYLEN ];    /* CDKEY */
  char passwd[ PASSWDLEN ];  /* 喇稀轡＆囁 */
  LoginType state;        /* 鰒時式健埼通周覗旒 */
  int nstatecount;
  char charname[ CHARNAMELEN ];  /* 健埼通周w式峠賭径o */
  int charaindex;     /* char?d樫式通周係喟柩稀z
                               * 健埼通周怨粟札必醒書直埖z-1札係易派誌傲
                               * ?d怨悧嶄掃z
                               */
  char CAbuf[ 2048 ];         /*  CA() 谷恂疎塋埖塵式弥喟易刹 */
  int CAbufsiz;       /*  CAbuf 式飯通宝  */

  struct timeval lastCAsendtime;     /*怨CA谷蜂堡俔掃筑 */

  char CDbuf[ 2048 ];         /*  CD() 谷恂疎塋埖塵式弥喟易刹 */
  int CDbufsiz;       /*  CDbuf 式飯通宝  */

  struct timeval lastCDsendtime;     /*怨CD谷蜂堡俔掃筑 */

  struct timeval lastCharSaveTime; /* 怨峠賭径係＆屎谷云＆討愼俔掃筑 */

  struct timeval lastprocesstime;    /* 怨駐健傲隷誌谷嵎A愼俔掃筑*/

  struct timeval lastreadtime;       /* 怨read愼俔掃筑z�怜郡俳札気*/
...
#define B3_TOLERANCE 5
 ...
 #define BEO_RESTORE 100
...
#ifdef _ITEM_ADDEXP //vincent 将刮戻N
  int EDTime;
#endif
  //    unsigned int      BEO;
  int BEOspool;
  ...
  int appendwb_overflow_flag;  /* 1者謄返appendWb札礎?愼俔晩1怨塋埖 */
  ...
  CONNECT_LOCK( sockfd );
{
  ...
  Connect[ sockfd ].close_request = 0;      /* 紿哮暸憩易径埼 */
...
#endif
#ifdef _ITEM_ADDEXP	//vincent 将刮戻幅
...
*/
	print( "圓埃 %d 俊銭...蛍塘 %.2f MB 腎寂...", size, (sizeof( CONNECT ) * size + RBSIZE * size + WBSIZE * size) / 1024.0 / 1024.0 );
...
{
//		print("\n購液萩箔譜崔葎 FILE:%s,LINE:%d ", file, line);
    CONNECT_LOCK(fd);
...
/*------------------------------------------------------------
 * charaindex 小晩 fd 谷  埖�
 *   遜嵎  式酎鶴喟柩谷梯畔愼俔�
 * 側佰
 *  charaindex      int     峠賭径式通周係喟柩稀
 * 潘垰粟
 *  易刹通誌係笛稀柩戀駐屎  圭堡晒� -1 式掃郡賞径＆
 ------------------------------------------------------------*/
        ...
        if ( chikulatime % 30 == 0 ) { // 耽300昼
          ...
          if ( CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORKITEM_ADDEXP ) > 0 &&
            ...
            if ( exptime <= 0 ) {
			  ...
			  CHAR_talkToCli( charaindex,-1,"戻幅僥楼将刮議嬬薦�払阻!",CHAR_COLORYELLOW);
            }
            else {
              CHAR_setWorkInt( charaindex, CHAR_WORKITEM_ADDEXPTIME, exptime );
              //print("\n 殊臥ADDEXPTIME %d ", exptime);
							
              if ( ( exptime % ( 60 * 60 ) ) < 300 && exptime >= ( 60 * 60 ) ) {
                char msg[ 1024 ];
				sprintf( msg, "戻幅僥楼将刮議嬬薦複寄埃 %d 弌扮。", (int)(exptime/(60*60)) );
				//sprintf( msg, "戻幅僥楼将刮議嬬薦複寄埃 %d 蛍。", (int)(exptime/(60)) );
                CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW );
              }
            }

#else
            if ( Connect[ i ].EDTime < CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORKITEM_ADDEXPTIME ) ) { //孔嬬恬喘嗤丼豚�
              ...
              CHAR_talkToCli( Connect[ i ].charaindex, -1, "戻N僥楼将刮議嬬薦�払阻!", CHAR_COLORYELLOW );
            }
          ...
          if ( CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORKITEMMETAMO ) < NowTime.tv_sec
            ...
            CHAR_setWorkInt( Connect[ i ].charaindex, CHAR_WORKNPCMETAMO, 0 ); //嚥npc斤三瘁議延附匆勣延指栖
            ...
            CHAR_talkToCli( Connect[ i ].charaindex, -1, "延附払丼阻。", CHAR_COLORWHITE );
          }

#endif
#ifdef _ITEM_TIME_LIMIT
          ITEM_TimeLimit( Connect[ i ].charaindex ); // (辛蝕慧) shan time limit of item. code:shan
...
#endif
#ifdef _CHIKULA_STONE
        if ( chikulatime % 3 == 0 && getChiStone( i ) > 0 ) { //徭強温僮
          CHAR_AutoChikulaStone( Connect[ i ].charaindex, getChiStone( i ) );
        }

#endif

        if ( chikulatime % 6 == 0 ) { //邦弊順彜蓑
          // Nuke 0626: No enemy
          ...
          if ( Connect[ i ].noenemy > 0 ) {
            ...
            if ( Connect[ i ].noenemy == 0 ) {
              CHAR_talkToCli( CONNECT_getCharaindex( i ), -1, "便擦�払阻。", CHAR_COLORWHITE );
            }
          }
        }

        //耽10昼
#ifdef _TYPE_TOXICATION //嶄蕎
...
*/
...
#ifdef _PETSKILL_BECOMEPIG
        /*if( CHAR_getInt( Connect[i].charaindex, CHAR_BECOMEPIG) > -1 ){ //侃豢鱗薦晒彜蓑
         if( ( CHAR_getInt( Connect[i].charaindex, CHAR_BECOMEPIG ) - 10 ) <= 0 ){ //鱗薦扮寂潤崩阻
          CHAR_setInt( Connect[i].charaindex, CHAR_BECOMEPIG, 0 );
          if( CHAR_getWorkInt( Connect[i].charaindex, CHAR_WORKBATTLEMODE ) == BATTLE_CHARMODE_NONE ){ //音頁壓媾況彜蓑和
              CHAR_setInt( Connect[i].charaindex, CHAR_BECOMEPIG, -1 );//潤崩鱗薦彜蓑
              CHAR_complianceParameter( Connect[i].charaindex );
              CHAR_sendCToArroundCharacter( CHAR_getWorkInt( Connect[i].charaindex , CHAR_WORKOBJINDEX ));
                 CHAR_send_P_StatusString( Connect[i].charaindex , CHAR_P_STRING_BASEBASEIMAGENUMBER);
              CHAR_talkToCli( Connect[i].charaindex,-1,"鱗薦晒払丼阻。",CHAR_COLORWHITE);
          }
         }
         else{
          char temp[256];
             ...
             if( CHAR_getWorkInt( Connect[i].charaindex, CHAR_WORKBATTLEMODE ) == BATTLE_CHARMODE_NONE ){ //音頁壓媾況彜蓑和
                                if( chikulatime%6 == 0 ){//60昼
                  sprintf(temp, "鱗薦扮寂:%d昼", CHAR_getInt( Connect[i].charaindex, CHAR_BECOMEPIG ));
        ...
        if ( CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORK_MAP_TIME ) > 0
          ...
          if ( CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORK_MAP_TIME ) <= 0 ) {
            // 扮寂欺阻,勧指秘笥
            CHAR_talkToCli( Connect[ i ].charaindex, -1, "低咀葎鞭音阻互犯遇犯棒�勧指叢血秘笥。", CHAR_COLORRED );
            ...
            char szMsg[ 64 ];
            sprintf( szMsg, "壓宸互犯議桟廠和低峪嬬壅棋 %d 昼。", CHAR_getWorkInt( Connect[ i ].charaindex, CHAR_WORK_MAP_TIME ) );
      ...
      if ( ( Connect[ i ].use ) && ( i != acfd )) {
        ...
        if ( CHAR_CHECKINDEX( Connect[ i ].charaindex ) )
          if ( CHAR_getInt( Connect[ i ].charaindex, CHAR_BECOMEPIG ) > -1 ) { //侃豢鱗薦晒彜蓑
            ...
            if ( ( CHAR_getInt( Connect[ i ].charaindex, CHAR_BECOMEPIG ) - 1 ) <= 0 ) { //鱗薦扮寂潤崩阻
              CHAR_setInt( Connect[ i ].charaindex, CHAR_BECOMEPIG, 0 );
...
*/
  ...
  if( ret > 0 && FD_ISSET(bindedfd , &rfds ) ){
    ...
    if( time_diff_us( et,st) >= looptime_us ) //峇佩耽階狛0.1昼彿勣恂議議垢恬
		{
      ...
      case 0:
        ...
        if ( !b_first_shutdown ) {
          ...
          i_shutdown_time = SERVSTATE_getLimittime();
		  		print("\n 購液捲暦匂扮寂=%d",i_shutdown_time);
        }
        break;
			default:
      {
					static int i_counter=0;
					// Syu ADD 協扮響函Announce
					static int j_counter=0;
					// Syu ADD 耽弌扮嶷仟厚仟哂俛媾皆電佩茨彿創
...
//			  int MaxItemNums;
					...
					if ( loop_counter > 60*getLoopAnnounceTime() && getLoopAnnounceMax()>0)
					{
					    ...
					    for( i = 0 ; i < playernum ; i++) {
					      if( CHAR_getCharUse(i) != FALSE ) {
					      	char buff[36];
					      	snprintf( buff, sizeof( buff),"%s巷御。",getGameserverID());
          ...
          /*
          #ifdef _ALLDOMAN     // Syu ADD 電佩茨NPC
          // Syu ADD 耽弌扮嶷仟厚仟哂俛媾皆電佩茨彿創
          if ( h_counter > 60*60 ){//36000 埃 3600昼=60蛍嶝
          h_counter=0;
          print("\nSyu log LoadHerolist");
          saacproto_UpdataStele_send ( acfd , "FirstLoad", "LoadHerolist" , "鯖吶" , 0 , 0 , 0 , 999 ) ;
          }
          #endif
          */
          if ( i_timeNu != time( NULL ) )  // 耽屁昼峇佩匯肝
          {
            i_timeNu = time( NULL );

#ifdef _DEL_DROP_GOLD
            //if( total_count % 60 == 0 ) { //耽蛍嶝峇佩
            // GOLD_DeleteTimeCheckLoop();
            //}
#endif
            //if( total_count % 60*10 == 0 ) { //耽10蛍嶝峇佩
            //}

            //if( total_count % 60*60 == 0 ) { //耽60蛍嶝峇佩
            //}
    ...
    if ( ret > 0 && FD_ISSET( fdremember, &rfds ) ) {
      ...
      if ( ret > 0 && sizeof( buf ) <= ret ) {
        print( "響函(%s)産喝海業:%d - %d !!\n", ( fdremember == acfd ) ? "SAAC" : "凪万", ret, sizeof( buf ) );
      }

			if( (ret == -1 && errno != EINTR) || ret == 0 ){
        if( fdremember == acfd ){
          print( "響函卦指:%d %s\n",ret,strerror(errno));
          print( "gmsv嚥acsv払肇銭俊! 呟械嶮峭...\n" );
          sigshutdown( -1 );
          exit(1);
        }else {
          if( ret == -1 ){
...
void sigusr2(int i)
{
  ...
  lssproto_Shutdown_recv( 0, "hogehoge", 5 ); // 5蛍嶝瘁略俐
}
...
BOOL CheckDefBTime( int charaindex, int fd, unsigned int lowTime, unsigned int battletime, unsigned int addTime)//lowTime决岳扮寂
{
  int delayTime = 0;
...
/*
  if ( NowTime < lowTime ) { //lowTime哘乎議媾況潤崩扮寂
    int r = 0;
...
*/
  //Connect[fd].BDTime = (NowTime+20)+delayTime;
	...
	if(getCharloops()>0)
  	Connect[fd].BDTime = NowTime + rand() % getCharloops() ; // 具右吉棋扮寂
  else
  	Connect[fd].BDTime = NowTime;
#else
  Connect[ fd ].BDTime = ( NowTime + rand() % 5 ) + delayTime + addTime; // 具右吉棋扮寂
#endif
  return TRUE;
}
#endif
