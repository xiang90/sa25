...
//				print("attvalue:%d \n", attvalue);
				...
				if(DefIsPlayer){	// 被打的是玩家
					...
					if(AttIsPlayer){	// 如果是玩家用魔法攻击
						...
						if ((float)def_magic_resist[FieldAttr] <= 0.0f) {
							temp = (float)att_magic_lv[FieldAttr] / 100;
						}
						else {
#endif
							temp = ((float)att_magic_lv[FieldAttr] - (float)def_magic_resist[FieldAttr])
