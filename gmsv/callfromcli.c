...
void lssproto_ClientLogin_recv( int fd,char* cdkey, char* passwd )
{
	...
	if (cdkey == NULL || passwd == NULL || cdkey[0] == '\0' || passwd[0] == '\0') {
		lssproto_ClientLogin_send (fd, "failed");
		return;
	}
    ...
    /* 忒蚕 */
#ifdef _ACNT_LOGIN
	saacproto_ACQueryAccount_send (acfd, cdkey, passwd, CONNECT_getFdid (fd), getServernumber ());
...
void lssproto_Shutdown_recv( int fd,char* passwd,int min )
{
	...
	if( strcmp( passwd, "hogehoge") == 0 ) {
		...
		snprintf( buff, sizeof( buff),"系统公告!");
	    for( i = 0 ; i < playernum ; i++) {
