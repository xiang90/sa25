...
{
	char servidchar[10];
	memset (servidchar, 0, sizeof (servidchar));
	sprintf (servidchar, "%d", servid);
	if (addWorkerTask (id, pas, "9", QueryAcctCallback, ti, id, pas, "", servidchar,"0", mesgid, 0) < 0) {
		saacproto_ACQueryAccount_send (ti, "failed", "server load too high", mesgid);
	}
}
#endif
...
{
	int i, fm_point, fm_tax;
    extern gmsv gs[MAXCONNECTION];

	for (i=0; i<MAXCONNECTION; i++) {
		if (gs[i].use && gs[i].name[0]) {
			for( fm_point = 0; fm_point < MAX_FMPOINT; fm_point++){
				fm_tax = GS_ASK_TAX( fm_point );							
				saacproto_GS_ACK_TAX_send(i, fm_tax, fm_point);
			}
		}
	}	
}
...
void saacproto_ACFMSetTAX_recv(int fd, int fm_tax, int index, int fmindex)
{
		...
		fm_point = ACFMSetTAX( fm_tax, fmindex);
		...
		{			
			for (i=0; i<MAXCONNECTION; i++) {
				if (gs[i].use && gs[i].name[0]) {
					saacproto_ACFMSetTAX_send(i, SUCCESSFUL, fm_tax, fm_point, index);
				}
			}	
		}else if( fm_point == -2) {
			saacproto_ACFMSetTAX_send(fd, FAILED, -1, 999, index);		// 不要怪我，不想再加栏位了，所以设成999
		}else{
			saacproto_ACFMSetTAX_send(fd, FAILED, -1, -1, index);		// -1 有许会有问题，要测试一下
		} 
...
void saacproto_ACLoadFmPk_recv(int fd, int fmpks_pos)
{
	extern char fm_pk_list[_FM_PK_LIST_NUM][255];
